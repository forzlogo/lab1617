...
class Validator {
public:
	static bool isValidRPN(const std::string& expression) {
		...
		return operandCount == operatorCount + 1 && operandCount > 0;
	}

	static bool isValidInfix(const std::string & expression) {
		int balance = 0;
		...
		return balance == 0 && !lastWasOperator && lastWasOperand;
	}

	static bool isTreeEmpty(void* root) {
		return root == nullptr;
	}
};
...
public:
	...
	void manualInput() {
		...
		if (!Validator::isValidRPN(expression)) {
			std::cout << "Ошибка\n";
			return;
		}
...
private:
	...
	void printTreeTilted(ExpressionNode * node, int level, std::ofstream & file, int indent = 4) {
		if (node == nullptr) return;

		printTreeTilted(node->right, level + 1, file, indent);
		...
		printTreeTilted(node->left, level + 1, file, indent);
	}
...
public:
	...
	void manualInput() {
		...
		if (!Validator::isValidInfix(expression)) {
			std::cout << "Ошибка\n";
			return;
		}
		buildFromString(expression);
	...
	void fileInput() {
		...
		if (!Validator::isValidInfix(line)) {
			std::cout << "Ошибка\n";
			return;
		}
		buildFromString(line);
	...
	void saveToFile(const std::string & filename, int x, int result) {
		std::ofstream file(filename);
		if (!file.is_open()) {
			std::cout << "Ошибка при создании файла " << filename << std::endl;
			return;
		}

		file << "Дерево после преобразования\n";

		if (root != nullptr) {
			printTreeTilted(root, 0, file, 4);
		}
	}
...
public:
	static void Execute() {
		...
		tree.saveToFile(outputFile, x, result);
	}
};
class CoutKotik {
public:
